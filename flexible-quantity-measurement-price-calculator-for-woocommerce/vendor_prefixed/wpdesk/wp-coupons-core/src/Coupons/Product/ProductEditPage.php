 ...
 */
namespace WDFQVendorFree\WPDesk\Library\WPCoupons\Product;
...
class ProductEditPage implements Hookable
{
    ...
    public function add_product_tab_action($tabs): array
    {
        foreach ($tabs as $tab_id => $tab) {
            $new_tabs[$tab_id] = $tab;
            if ($tab_id === 'general') {
                $new_tabs[self::TAB_KEY] = ['label' => esc_html__('PDF Coupon', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'target' => 'pdfcoupon_product_data', 'class' => ['hide_if_grouped', 'hide_if_external', 'hide_if_coupon_disabled', 'hide'], 'priority' => 12];
            }
        }
        return $new_tabs;
    }
    ...
    {
        $types['wpdesk_pdf_coupons'] = ['id' => self::PRODUCT_COUPON_SLUG, 'wrapper_class' => 'show_if_simple', 'label' => esc_html__('PDF Coupon', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'description' => esc_html__('Convert this product to PDF Coupon', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'default' => 'no'];
        return $types;
    }
    /**
     * Update product type.
