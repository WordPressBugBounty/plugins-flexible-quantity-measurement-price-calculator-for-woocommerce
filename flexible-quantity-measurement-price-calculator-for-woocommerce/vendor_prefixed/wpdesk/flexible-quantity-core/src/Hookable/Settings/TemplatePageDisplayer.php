...
class TemplatePageDisplayer implements \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var Renderer
     */
     ...
     */
    private $is_locked;
    public function __construct(\WDFQVendorFree\WPDesk\View\Renderer\Renderer $renderer, \WDFQVendorFree\WPDesk\Translation\Translate $translate, bool $is_locked)
    {
    ...
    public function render_selection($post)
    {
        ...
        $selection_category = \get_post_meta($post->ID, \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\TemplatePageSaver::SELECTION_CATEGORY_META_KEY, \true);
        ...
        $this->renderer->output_render('settings/selection-section', ['selection_category' => $selection_category, 'pre_select_product_id' => $pre_select_product_id, 'nonce_name' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\TemplatePageSaver::NONCE_NAME, 'nonce_action' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\TemplatePageSaver::NONCE_ACTION, 'translate' => $this->translate]);
    }
    /**
     * @param \WP_Post $post Post object.
     *
    ...
    public function render_template($post)
    {
        ...
        $this->renderer->output_render('settings/template-section', ['settings' => $settings_bag, 'translate' => $this->translate, 'renderer' => $this->renderer, 'is_locked' => $this->is_locked, 'available_units' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Units::get_all(), 'current_currency' => \get_woocommerce_currency_symbol()]);
    }
    private function get_create_template_from_product_query_var() : int
    {
        if (empty($_GET[\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\ProductPage::CREATE_FROM_PRODUCT_VAR_NAME])) {
            return 0;
        }
        return \absint($_GET[\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\ProductPage::CREATE_FROM_PRODUCT_VAR_NAME]);
    }
    private function get_assign_product_to_template_query_var() : int
    {
        if (empty($_GET[\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\ProductPage::ASSIGN_PRODUCT_VAR_NAME])) {
            return 0;
        }
        return \absint($_GET[\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\ProductPage::ASSIGN_PRODUCT_VAR_NAME]);
    }
