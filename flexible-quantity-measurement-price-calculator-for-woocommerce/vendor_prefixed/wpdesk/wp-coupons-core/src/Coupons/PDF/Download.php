...
class Download implements Hookable
{
    ...
    public function get_pdf_content($request = [])
    {
        ...
        if (!isset($coupon_data['hash'])) {
            wp_die(esc_html__('This coupon not exits or expired!', 'flexible-quantity-measurement-price-calculator-for-woocommerce'));
        }
        ...
        try {
            ...
            if (!isset($request['view'])) {
                ...
                $name = $code . '.pdf';
                $prefix = esc_html__('Coupon', 'flexible-quantity-measurement-price-calculator-for-woocommerce');
