...
class SupportPage implements Hookable
{
    ...
    public function __construct(Renderer $renderer, Translate $translate, string $assets_url, string $script_version, string $marketing_slug, bool $is_locked)
    {
    ...
    {
        \add_submenu_page('edit.php?post_type=' . FQTemplateType::POST_TYPE, esc_html__('Start here', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), '<span style="color: #00FFC2;">' . esc_html__('Start here', 'flexible-quantity-measurement-price-calculator-for-woocommerce') . '</span>', 'manage_options', self::MENU_SLUG, [$this, 'render'], self::MENU_POSITION);
    }
    public function render()
    {
        ...
        $boxes = new MarketingBoxes($this->marketing_slug, $local);
    ...
    {
        if (!in_array($screen_id, [FQTemplateType::POST_TYPE . '_page_' . self::MENU_SLUG], \true)) {
            return;
        }
