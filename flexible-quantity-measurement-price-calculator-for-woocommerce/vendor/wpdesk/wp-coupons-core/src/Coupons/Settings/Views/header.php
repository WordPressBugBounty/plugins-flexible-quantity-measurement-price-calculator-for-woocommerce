...
<tr>
	<td class="header-column" colspan="2">
		...
		if ( $field->has_label() ) {
			?>
			<h<?php echo (int) $header_size; ?>	<?php $field->has_classes() ? ' class="' . \esc_attr( $field->get_classes() ) . '"' : ''; ?>>
				<?php echo \esc_html( $field->get_label() ); ?>
			</h<?php echo (int) $header_size; ?>>
			<?php
		}
		if ( $field->has_description() ) {
			?>
			<p <?php $field->has_classes() ? ' class="' . \esc_attr( $field->get_classes() ) . '"' : ''; ?>>
				<?php
