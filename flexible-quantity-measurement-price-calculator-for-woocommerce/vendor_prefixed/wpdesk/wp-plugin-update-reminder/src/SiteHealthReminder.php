...
class SiteHealthReminder implements Reminder, Hookable
{
    ...
    {
        $results = ['label' => __('You should update plugin', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'status' => $status, 'badge' => ['label' => __('Compatibility', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'color' => $color], 'description' => $description, 'actions' => '', 'test' => basename($this->reminder_data->get_plugin_dir())];
        return $results;
    }
}
