 ...
 */
namespace WDFQVendorFree\WPDesk\Library\WPCoupons\Coupon;
...
use WDFQVendorFree\WPDesk\Library\WPCoupons\Exception\EmailException;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Email\FlexibleCouponsBaseEmail;
...
class GenerateCoupon implements Hookable
{
    ...
    public function wp_ajax_generate_coupon()
    {
        if (!isset($_POST['security']) || !wp_verify_nonce($_POST['security'], 'generate_coupon')) {
            ...
            // WPCS: CSRF ok, input var ok.
            if (!$order_id || !$item_id) {
                wp_send_json_error(esc_html__('Cannot generate coupon. Unknown order ID or item ID', 'flexible-quantity-measurement-price-calculator-for-woocommerce'));
            }
            $order = wc_get_order($order_id);
            $item = $order->get_item($item_id);
            try {
                ...
                if (!empty($coupon)) {
                    ...
                    wp_send_json_error(esc_html__('Cannot generate coupon', 'flexible-quantity-measurement-price-calculator-for-woocommerce'));
                }
            } catch (Exception $e) {
