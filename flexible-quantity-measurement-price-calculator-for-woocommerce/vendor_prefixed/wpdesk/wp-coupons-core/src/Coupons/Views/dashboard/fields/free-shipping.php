...
if (\wc_shipping_enabled()) {
    \woocommerce_wp_checkbox(['id' => 'fc_product_free_shipping' . $loop_id, 'name' => 'fc_product_free_shipping' . $loop_name, 'value' => $value, 'label' => \esc_html__('Allow free shipping', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'desc_tip' => \true, 'description' => \esc_html__('Check if you want the coupon to provide free shipping. The coupon requirement for free shipping must be included in the shipping method.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'wrapper_class' => !$is_premium ? 'read-only' : '']);
}
