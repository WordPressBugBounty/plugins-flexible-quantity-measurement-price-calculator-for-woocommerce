...
use WPDesk\Library\WPCoupons\Helpers\Plugin;
...
$style      = 'display: none;';
$is_enabled = 'yes' === get_post_meta( $prod_post_id, '_flexible_coupon_variation_base_on', true );
if ( $is_enabled ) {
	$style = 'display: block !important;';
}
?>

<div id="pdfcoupon_product_data_variation" class="show_if_variation_manage_coupons" style="<?php echo \esc_attr( $style ); ?>">
	...
	<p class="form-field fc_coupon_code_field coupon-code-settings">
		<label for="fc_coupon_code">
			...
			/* translators: %1$s: anchor opening tag, %2$s: anchor closing tag */
				esc_html__( 'Read the %1$splugin documentation →%2$s', 'wp-coupons-core' ),
	...
	<div class="fc-options-group fc-custom-fields-group">
		...
		if ( ! $is_premium ) {
			...
			printf(
			/* translators: %1$s: anchor opening tag, %2$s: anchor closing tag */
				esc_html__( '%1$sUpgrade to PRO →%2$s and enable options below', 'wp-coupons-core' ),
				sprintf(
					'<a href="%s" target="_blank" class="pro-link">',
					esc_url( $pro_url ) . '&utm_content=edit-product'
				),
				'</a>'
			);
			echo '</p>';
		...
		if ( ! $is_multiple_pdfs ) {
			...
			printf(
			/* translators: %1$s: anchor opening tag, %2$s: anchor closing tag */
				esc_html__( 'Buy %1$sFlexible PDF Coupons PRO - Multiple PDFs →%2$s and enable options below', 'wp-coupons-core' ),
				sprintf(
					'<a href="%s" target="_blank" class="sending-link">',
					\esc_url( Links::get_fcmpdf_link() ) . '&utm_content=&utm_content=edit-product'
				),
				'</a>'
			);
			echo '</p>';
			...
			$renderer->output_render(
				'fields/multiple-pdfs/multiple-coupons-enable',
				[
					'post_meta'         => $post_meta,
					'post_id'           => $prod_post_id,
					'is_premium'        => $is_premium,
					'is_multiple_pdfs'  => $is_multiple_pdfs,
					'custom_attributes' => $custom_attributes,
					'settings'          => $settings,
					'loop'              => $loop,
				]
			...
			<div class="fc-options-group fc-custom-fields-group fc-multiple-pdfs-advanced-options">
				<?php
				$renderer->output_render(
					'fields/multiple-pdfs/multiple-coupons-send-to-first-email',
					[
						'post_meta'         => $post_meta,
						'post_id'           => $prod_post_id,
						'is_premium'        => $is_premium,
				...
				$renderer->output_render(
					'fields/multiple-pdfs/multiple-coupons-forms-limit',
					[
						'post_meta'         => $post_meta,
						'post_id'           => $prod_post_id,
						'is_premium'        => $is_premium,
		...
		?>
		<div class="show_if_variation_manage_prefix" style="<?php echo \esc_attr( $style ); ?>">
			<?php
			$renderer->output_render(
				'fields/coupon-code-prefix',
				[
					'post_meta'         => $post_meta,
					'post_id'           => $prod_post_id,
					'parent_id'         => $parent_id,
	...
	<div class="fc-options-group">
		...
		if ( ! $is_sending ) {
			...
			printf(
			/* translators: %1$s: anchor opening tag, %2$s: anchor closing tag */
				esc_html__( 'Buy %1$sFlexible PDF Coupons PRO - Advanced Sending →%2$s and enable options below', 'wp-coupons-core' ),
				sprintf(
					'<a href="%s" target="_blank" class="sending-link">',
					\esc_url( Links::get_fcs_link() ) . '&utm_content=&utm_content=edit-variations'
				),
				'</a>'
			);
			echo '</p>';
