...
class CalculatorPriceAjax implements \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
    ...
    public function __construct(\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\SettingsContainer $settings_container, \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Prices\PriceModifier $price_modifier)
    {
    ...
    public function price_calculation()
    {
        ...
        $validator = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\Validator\MeasurementValidator(new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\Validator\PositiveNumberValidator(), new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\Validator\InRangeValidator($settings), new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\Validator\IncrementValidator($settings));
        $result = $validator->validate($measurements);
        if (!$result->is_valid()) {
            \wp_send_json_error($result->get_errors());
        }
        $measurement_calculator = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\Calculator\MeasurementCalculator($settings);
        $measurement = $measurement_calculator->calculate($measurements);
        $price_calculator = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Services\Calculator\PriceCalculator($settings);
