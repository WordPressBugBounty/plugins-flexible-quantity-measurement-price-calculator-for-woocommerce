...
<p class="form-field">
	<label
		for="fc_product_categories<?php echo \esc_attr( $loop_id ); ?>"><?php esc_html_e( 'Product categories', 'wp-coupons-core' ); ?></label>
	<select
		id="fc_product_categories<?php echo \esc_attr( $loop_id ); ?>"
		name="fc_product_categories<?php echo \esc_attr( $loop_name ); ?>[]"
		class="wc-enhanced-select"
		style="width: 80% !important;"
		multiple="multiple" data-placeholder="<?php esc_attr_e( 'Any category', 'wp-coupons-core' ); ?>">
		<?php
		...
		?>
	</select> <?php echo wc_help_tip( esc_html__( 'Categories for which the coupon will be used. Do not select any to apply to all categories.', 'wp-coupons-core' ) ); ?>
</p>
