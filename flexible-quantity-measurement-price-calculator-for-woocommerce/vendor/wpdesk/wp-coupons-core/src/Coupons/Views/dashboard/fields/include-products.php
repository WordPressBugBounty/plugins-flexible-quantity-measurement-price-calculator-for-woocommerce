...
?>
<p class="form-field">
	<label for="fc_product_ids<?php echo \esc_attr( $loop_id ); ?>"><?php esc_html_e( 'Products', 'wp-coupons-core' ); ?></label>
	<select
		id="fc_product_ids<?php echo \esc_attr( $loop_id ); ?>"
		class="wc-product-search" multiple="multiple"
		name="fc_product_ids<?php echo \esc_attr( $loop_name ); ?>[]"
		style="width: 80% !important;"
		data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'wp-coupons-core' ); ?>"
		...
		foreach ( $product_ids as $product_id ) {
			...
			if ( is_object( $product ) ) {
				echo '<option value="' . esc_attr( $product_id ) . '" ' . selected( true, true, false ) . '>' . \wp_kses_post( \htmlspecialchars( $product->get_formatted_name() ) ) . '</option>';
			}
		}
		?>
	</select>
	<?php echo wc_help_tip( esc_html__( 'Products for which the coupon will be used. Do not select any to apply to all products.', 'wp-coupons-core' ) ); ?>
</p>
