...
namespace WPDesk\Library\WPCoupons\Cart;
...
use WPDesk\Library\WPCoupons\Product\ProductEditPage;
...
class Cart implements Hookable {
	...
	public function __construct(
		...
		PostMeta $post_meta,
		WordpressOptionsContainer $settings,
	...
	public function add_fields_to_product_action() {
		...
		try {
			...
			$variation_id = isset( $_POST['variation_id'] ) ? (int) $_POST['variation_id'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing
			if ( $variation_id ) {
	...
	private function validate_field( $product_id, array $field, $value ) {
		...
		try {
			...
			if ( empty( $required ) ) {
				$is_required = $field['required']; // This option is defined in field declaration.
			} else {
	...
	private function should_throw_exception_is_empty( $field, $value ) {
		if ( empty( $value ) ) {
			...
			throw new Exception( sprintf( __( '<strong>%s</strong>  is required field.', 'wp-coupons-core' ), $field['title'] ) );
		}
	}
	...
	private function should_throw_exception_for_minlength( $field, $value ) {
		if ( isset( $field['validation']['minlength'] ) && mb_strlen( trim( $value ) ) < (int) $field['validation']['minlength'] ) {
			// translators: field title.
			throw new Exception( sprintf( __( '<strong>%1$s</strong>  is to short. Minimum length: %2$d.', 'wp-coupons-core' ), $field['title'], $field['validation']['minlength'] ) );
		}
	}
	...
	private function should_throw_exception_for_maxlength( $field, $value ) {
		if ( isset( $field['validation']['maxlength'] ) && mb_strlen( trim( $value ) ) > (int) $field['validation']['maxlength'] ) {
			// translators: field title.
			throw new Exception( sprintf( __( '<strong>%1$s</strong>  is to long. Maximum length: %2$d.', 'wp-coupons-core' ), $field['title'], $field['validation']['maxlength'] ) );
		}
	}
	...
	private function should_throw_exception_for_email( $field, $value ) {
		if ( isset( $field['validation']['email'] ) && ! is_email( $value ) ) {
			// translators: field title.
			throw new Exception( sprintf( __( '<strong>%s</strong>  has invalid email.', 'wp-coupons-core' ), $field['title'] ) );
		}
	}
	...
	private function should_throw_exception_for_invalid_date( $field, $value ) {
		if ( isset( $field['validation']['date'] ) && strtotime( $value ) === false ) {
			// translators: field title.
			throw new Exception( sprintf( __( '<strong>%s</strong> has an invalid date.', 'wp-coupons-core' ), $field['title'] ) );
		}
	}
	...
	private function should_throw_exception_for_past_date( $field, $value ) {
		if ( isset( $field['validation']['past_date'] ) && strtotime( $value ) < \time() ) {
			// translators: field title.
			throw new Exception( sprintf( __( '<strong>%s</strong> has a date from the past.', 'wp-coupons-core' ), $field['title'] ) );
		}
	}
			...
			if ( ! empty( $cart_item['flexible_coupons'] ) ) {
				foreach ( $cart_item['flexible_coupons'] as $field ) {
					...
					if ( ! empty( $value ) ) {
						$item_data[] = [
							'name'    => $name,
							'value'   => wp_strip_all_tags( $field['value'] ),
