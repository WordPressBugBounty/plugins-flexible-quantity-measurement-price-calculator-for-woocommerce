...
namespace WPDesk\Library\WPCoupons\Product;
...
class ProductEditPage implements Hookable {
	...
	public function add_product_tab_action( $tabs ): array {
		foreach ( $tabs as $tab_id => $tab ) {
			$new_tabs[ $tab_id ] = $tab;
			if ( $tab_id === 'general' ) {
				$new_tabs[ self::TAB_KEY ] = [
					'label'    => esc_html__( 'PDF Coupon', 'wp-coupons-core' ),
					'target'   => 'pdfcoupon_product_data',
					'class'    => [ 'hide_if_grouped', 'hide_if_external', 'hide_if_coupon_disabled', 'hide' ],
					'priority' => 12,
				];
			}
		}
	...
	public function add_product_general_data_field() {
		...
		$this->renderer->output_render(
			'html-product-general-settings',
			[
				'renderer'                    => $this->renderer,
				'nonce_name'                  => self::NONCE_NAME,
				'nonce_action'                => self::NONCE_ACTION,
				'is_premium'                  => $this->product_fields->is_premium(),
				'product_fields'              => $this->product_fields,
				'product_templates'           => $this->get_coupons_templates_options(),
				'post_meta'                   => $this->post_meta,
				'post_id'                     => $post->ID,
				'self'                        => $this,
				'expiring_date_default_value' => $expiring_date_default_value,
				'custom_attributes'           => $this->get_field_attributes(),
	...
	public function add_product_type_filter( array $types ): array {
		$types['wpdesk_pdf_coupons'] = [
			'id'            => self::PRODUCT_COUPON_SLUG,
			'wrapper_class' => 'show_if_simple',
			'label'         => esc_html__( 'PDF Coupon', 'wp-coupons-core' ),
			'description'   => esc_html__( 'Convert this product to PDF Coupon', 'wp-coupons-core' ),
			'default'       => 'no',
		];
	...
	private function get_coupons_templates_options(): array {
		...
		$posts = get_posts(
			[
				'post_type'      => $this->editor_post_type,
				'post_status'    => 'publish',
				'posts_per_page' => '-1',
			]
		);
