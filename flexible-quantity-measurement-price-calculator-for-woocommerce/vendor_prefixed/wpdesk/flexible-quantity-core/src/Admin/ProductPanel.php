...
class ProductPanel implements Hookable
{
    ...
    function admin_enqueue_scripts()
    {
        ...
        if ($screen && 'product' === $screen->id) {
            wp_enqueue_script('fq-input-filter', $this->plugin_url . '/assets/js/input-filter.js', [], PluginConfig::get_script_version());
            wp_enqueue_script('jquery-validate', $this->plugin_url . '/assets/js/vendor/jquery.validate.min.js', ['jquery'], PluginConfig::get_script_version());
            wp_enqueue_script('fq-admin', $this->plugin_url . '/assets/js/admin/fq-price-calculator.min.js', ['jquery-validate', 'jquery'], PluginConfig::get_script_version());
            wp_enqueue_script('fq-admin-modal', $this->plugin_url . '/assets/js/modal.js', ['jquery-validate', 'jquery'], PluginConfig::get_script_version());
            // Variables for JS scripts
            $fq_admin_params = ['nonce' => wp_create_nonce(self::FQ_AJAX_ADMIN_NONCE), 'get_dimensions' => self::FQ_AJAX_ACTION, 'product_id' => $post->ID, 'loader_img' => '<img class="center" src="' . esc_url(includes_url() . 'js/tinymce/skins/lightgray/img/loader.gif') . '" />', 'server_error' => __('We noticed an error while trying to get the dimension table.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'input_filter' => __('Only digits allowed.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'woocommerce_currency_symbol' => get_woocommerce_currency_symbol(), 'woocommerce_weight_unit' => 'no' !== get_option('woocommerce_enable_weight', \true) ? get_option('woocommerce_weight_unit') : '', 'pricing_rules_enabled_notice' => __('Cannot edit price while a pricing table is active', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'is_variable_product_with_stock_managed' => Helper::is_variable_product_with_stock_managed(wc_get_product($post))];
            wp_localize_script('fq-admin', 'fq_admin_params', $fq_admin_params);
            \wp_enqueue_style('fq-admin-css', $this->plugin_url . '/assets/css/admin.css', [], PluginConfig::get_script_version());
        }
        \wp_enqueue_style('fq-admin-marketing-modal-css', $this->plugin_url . '/assets/css/modal.css', [], PluginConfig::get_script_version());
        \wp_enqueue_style('fq-admin-marketing-css', $this->plugin_url . '/assets/css/marketing.css', [], PluginConfig::get_script_version());
    }
    /**
