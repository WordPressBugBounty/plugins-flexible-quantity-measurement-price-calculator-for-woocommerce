...
namespace WPDesk\Library\WPCoupons\Coupon;
...
use WPDesk\Library\WPCoupons\Data\Email\EmailMeta;
...
use WPDesk\Library\WPCoupons\Exception\EmailException;
use WPDesk\Library\WPCoupons\Email\FlexibleCouponsBaseEmail;
...
class GenerateCoupon implements Hookable {
	...
	public function create_coupon_for_order_status( int $order_id ) {
		try {
			...
			$this->logger->warning(
				sprintf(
					'Failed to generate coupon for order %d. Original error: %s',
					$order_id,
					$e->getMessage()
				)
			);
	...
	private function should_send_email( int $order_id, array $meta ): void {
		...
		foreach ( $wc_registered_emails as $email ) {
			...
			try {
				...
				$should_send = \apply_filters( 'fc/core/email/should_send_email', true, $email, $order_id, $meta, $meta_data );
	...
	public function delete_order_item_postmeta( int $coupon_id ) {
		...
		$meta = $this->postmeta->get_private( $coupon_id, 'fcpdf_coupon_data', true );
    ...
    public function wp_ajax_generate_coupon() {
        if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'generate_coupon' ) ) {
            ...
            $order_id = isset( $_POST['order_id'] ) ? (int) $_POST['order_id'] : false; // WPCS: CSRF ok, input var ok.
            $item_id  = isset( $_POST['item_id'] ) ? (int) $_POST['item_id'] : false; // WPCS: CSRF ok, input var ok.
            if ( ! $order_id || ! $item_id ) {
                wp_send_json_error( esc_html__( 'Cannot generate coupon. Unknown order ID or item ID', 'wp-coupons-core' ) );
            }
            ...
            try {
                ...
                if ( ! empty( $coupon ) ) {
                    wp_send_json_success(
                        [
                            'html' => $this->get_success_html( $coupon ),
                        ]
                    );
                } else {
                    wp_send_json_error( esc_html__( 'Cannot generate coupon', 'wp-coupons-core' ) );
                }
            } catch ( Exception $e ) {
