...
namespace WPDesk\Library\WPCoupons\PDF;

use Exception;
use RuntimeException;
use WC_Coupon;
use WPDesk\Library\CouponInterfaces\EditorIntegration;
use WPDesk\Library\WPCoupons\Integration\Helper;
...
use WPDesk\Library\WPCoupons\Shortcodes\Shortcodes;
use WPDesk\Library\WPCoupons\Shortcodes\ShortcodeDataContainer;
use WPDesk\View\Renderer\Renderer;
use WC_Order;
use WC_Order_Item;
use WC_Order_Item_Product;
use WC_Product;
use WPDesk\Library\WPCoupons\Integration\ShortCodeReplacer;
...
class PDF {
	...
	private function get_order_item_meta( WC_Order $order, int $item_id ): array {
		...
		if ( $item instanceof WC_Order_Item_Product ) {
			$is_coupon_item = 'yes' === $this->postmeta->get_private( $item->get_product_id(), ProductEditPage::PRODUCT_COUPON_SLUG, true );
			if ( $is_coupon_item && $this->product_fields->is_premium() ) {
				foreach ( $this->product_fields->get() as $id => $field ) {
					$meta[ $id ] = wc_get_order_item_meta( $item->get_id(), $id, true );
				}
			}
		}
	...
	private function prepare_template_data( int $template_id, WC_Order $order, WC_Order_Item $item, array $product_fields_values ): array {
		...
		if ( $item instanceof WC_Order_Item_Product ) {
			...
			if ( empty( $template_meta ) || ! is_array( $template_meta ) ) {
				throw new RuntimeException( esc_html__( 'Unknown template', 'wp-coupons-core' ) );
			}

			$product = $item->get_product();
			$coupon  = new WC_Coupon( $coupon_id );
			if ( ! $coupon->get_id() ) {
				throw new RuntimeException( esc_html__( 'Coupon doesn\'t exists.', 'wp-coupons-core' ) );
			}
	...
	public function string_output( $order_id, $item_id ): string {
		...
		$data            = [
			'editor_width'   => $area_properties->get_width(),
			'editor_height'  => $area_properties->get_height(),
			'editor_bgcolor' => $area_properties->get_background_color(),
			'html'           => $editor_objects->get_html(),
		];
