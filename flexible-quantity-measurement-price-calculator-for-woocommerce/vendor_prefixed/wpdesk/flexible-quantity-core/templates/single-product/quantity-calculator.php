 ...
 */
global $product;
$actual_amount_text = \apply_filters('fq_price_calculator_actual_amount_text', $product_measurement->get_unit_label() ? \sprintf(\__('Actual %1$s (%2$s)', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $product_measurement->get_label(), $product_measurement->get_unit_label()) : \sprintf(\__('Actual %s', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $product_measurement->get_label()));
?>
...
foreach ($measurements as $measurement) {
    ...
    if ($measurement->is_editable()) {
        ...
        if ($measurement->get_unit_label()) {
            ...
            echo \wp_kses_post(\sprintf(\__('%1$s (%2$s)', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $measurement->get_label(), $measurement->get_unit_label()));
        } else {
