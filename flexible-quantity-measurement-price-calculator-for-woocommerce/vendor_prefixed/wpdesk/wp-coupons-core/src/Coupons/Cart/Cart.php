 ...
 */
namespace WDFQVendorFree\WPDesk\Library\WPCoupons\Cart;
...
use WDFQVendorFree\WPDesk\Library\WPCoupons\Product\ProductEditPage;
use WDFQVendorFree\WPDesk\Persistence\Adapter\WordPress\WordpressOptionsContainer;
...
class Cart implements Hookable
{
    ...
    private function should_throw_exception_is_empty($field, $value)
    {
        if (empty($value)) {
            ...
            throw new Exception(sprintf(__('<strong>%s</strong>  is required field.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $field['title']));
        }
    }
    /**
     * @throws Exception Throw error exception.
    ...
    private function should_throw_exception_for_minlength($field, $value)
    {
        if (isset($field['validation']['minlength']) && mb_strlen(trim($value)) < (int) $field['validation']['minlength']) {
            // translators: field title.
            throw new Exception(sprintf(__('<strong>%1$s</strong>  is to short. Minimum length: %2$d.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $field['title'], $field['validation']['minlength']));
        }
    }
    /**
     * @throws Exception Throw error exception.
    ...
    private function should_throw_exception_for_maxlength($field, $value)
    {
        if (isset($field['validation']['maxlength']) && mb_strlen(trim($value)) > (int) $field['validation']['maxlength']) {
            // translators: field title.
            throw new Exception(sprintf(__('<strong>%1$s</strong>  is to long. Maximum length: %2$d.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $field['title'], $field['validation']['maxlength']));
        }
    }
    /**
     * @throws Exception Throw error exception.
    ...
    private function should_throw_exception_for_email($field, $value)
    {
        if (isset($field['validation']['email']) && !is_email($value)) {
            // translators: field title.
            throw new Exception(sprintf(__('<strong>%s</strong>  has invalid email.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $field['title']));
        }
    }
    /**
     * @throws Exception Throw error exception.
    ...
    private function should_throw_exception_for_invalid_date($field, $value)
    {
        if (isset($field['validation']['date']) && strtotime($value) === \false) {
            // translators: field title.
            throw new Exception(sprintf(__('<strong>%s</strong> has an invalid date.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $field['title']));
        }
    }
    /**
     * @throws Exception Throw error exception.
    ...
    private function should_throw_exception_for_past_date($field, $value)
    {
        if (isset($field['validation']['past_date']) && strtotime($value) < \time()) {
            // translators: field title.
            throw new Exception(sprintf(__('<strong>%s</strong> has a date from the past.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $field['title']));
        }
    }
    /**
     * Save product fields.
