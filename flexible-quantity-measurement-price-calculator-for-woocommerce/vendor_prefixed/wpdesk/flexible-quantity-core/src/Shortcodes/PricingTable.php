...
class PricingTable
{
    ...
    public static function get_pricing_rules_table($rules, $settings)
    {
        ...
        $html .= '<thead><tr><th>' . sprintf(__('Range (%s)', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), '<span class="units">' . $settings->get_pricing_label() . '</span>') . '</th>';
        /* translators: Placeholders: %s - currency symbol */
        $html .= '<th>' . sprintf(__('Price (%s)', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), '<span class="units">' . get_woocommerce_currency_symbol() . '/' . $settings->get_pricing_label() . '</span>') . '</th></tr></thead>';
        $html .= '<tbody>';
        foreach ($rules as $rule) {
            ...
            $range .= ' ' . $settings->get_pricing_label();
