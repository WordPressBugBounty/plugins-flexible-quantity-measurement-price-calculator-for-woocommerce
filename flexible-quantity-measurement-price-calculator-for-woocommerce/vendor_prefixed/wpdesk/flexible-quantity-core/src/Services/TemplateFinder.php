...
class TemplateFinder
{
    ...
    {
        $args = ['post_type' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\PostType\FQTemplateType::POST_TYPE, 'meta_query' => ['relation' => 'AND', ['key' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Settings::SETTINGS_META_KEY . '_hash', 'value' => \md5(\wp_json_encode($settings))], ['key' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\TemplatePageSaver::SELECTION_CATEGORY_META_KEY, 'value' => 'products']]];
    ...
    {
        $args = ['post_type' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\PostType\FQTemplateType::POST_TYPE, 'post_status' => 'publish', 'meta_query' => [['key' => 'product_id', 'value' => $product_id]]];
        $query = new \WP_Query($args);
        if ($query->post_count > 0) {
            return $query->posts[0]->ID;
        }
    ...
    private function get_template_by_meta_key(string $meta_key, array $meta_values) : int
    {
        ...
        $args = ['post_type' => \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\PostType\FQTemplateType::POST_TYPE, 'post_status' => 'publish', 'meta_query' => [['key' => $meta_key, 'value' => $meta_values, 'compare' => 'IN']]];
        $query = new \WP_Query($args);
        if ($query->post_count > 0) {
            return $query->posts[0]->ID;
        }
