...
namespace WPDesk\Library\WPCoupons\Order;
...
class OrderMetaBox implements Hookable {
		...
		if ( $this->is_order_page() ) {
			...
			if ( $order instanceof \WC_Order ) {
				...
				foreach ( $products as $key => $items ) {
					...
					if ( $is_coupon_item ) {
						\add_meta_box(
							'flexible_coupon__' . $key,
							\__( 'PDF Coupon', 'wp-coupons-core' ),
							[ $this, 'order_coupon_callback' ],
							$screens,
							self::META_BOX_CONTEXT_SIDE,
							self::META_BOX_PRIORITY_LOW,
							[
								'post_id' => $order_id,
								'items'   => $items,
							]
	...
	public function order_coupon_callback( $post_or_order_object, $args ) {
		...
		foreach ( $items as $item ) {
			...
			if ( $this->should_send_mail_to_first_recipient( $product_id ) ) { // todo: magic string. use const and filter var
	...
	private function get_item_render_data( WC_Order_Item $item, WC_Order $order ): array {
		...
		return [
			...
			'coupon_id'             => $coupon->get_id(), // TODO
			'coupon_url'            => $coupon_url, // TODO
			'coupon_code'           => $coupon_code, // TODO
			'coupon_is_used'        => $this->is_coupon_limit_reached( $coupon ),
			'coupon_title'          => $coupon_code,
			'download_url'          => $download_url, // TODO
