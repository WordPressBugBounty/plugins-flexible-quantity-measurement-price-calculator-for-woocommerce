...
namespace WPDesk\Library\WPCoupons\Email;
...
class FlexibleCouponsBaseEmail extends WC_Email implements Email {
	...
	public function get_content_html(): string {
		...
		wc_get_template(
			...
			[
				...
				'date_order'     => $this->get_date_order(),
				'sent_to_admin'  => false,
				'plain_text'     => false,
			],
			'',
	...
	public function init_form_fields() {
		...
		$this->form_fields = [
			...
			'email_type' => [
				...
				'class'       => 'email_type',
				'options'     => [
					'plain'     => esc_html__( 'Plain text', 'woocommerce' ),
					'html'      => esc_html__( 'HTML', 'woocommerce' ),
					'multipart' => esc_html__( 'Multipart', 'woocommerce' ),
				],
			],
		];
	...
	protected function setup_placeholders() {
		$this->placeholders = array_merge(
			[
				'{order_date}'   => $this->get_date_order(),
				'{order_number}' => $this->object->get_order_number(),
			],
			$this->placeholders
		);
