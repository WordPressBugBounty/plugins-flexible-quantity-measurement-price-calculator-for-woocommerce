...
namespace WDFQVendorFree\WPDesk\Library\WPCoupons\Settings;

use WDFQVendorFree\WPDesk\Library\WPCoupons\CouponsIntegration;
...
use WDFQVendorFree\WPDesk\Forms\Resolver\DefaultFormFieldResolver;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Settings\Tabs\SettingsTab;
/**
...
class SettingsForm implements Hookable
{
    ...
    public function hooks()
    {
        add_action('admin_menu', function () {
            add_submenu_page(self::MENU_PAGE_URL, esc_html__('Settings', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), esc_html__('Settings', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'manage_options', self::SETTINGS_SLUG, [$this, 'render_page_action'], 40);
        }, 999);
    ...
    public function save_settings_action()
    {
        ...
        if (!empty($tab_data) && $nonce) {
            ...
            new Notice(esc_html__('Your settings have been saved.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), Notice::NOTICE_TYPE_SUCCESS);
        } else {
