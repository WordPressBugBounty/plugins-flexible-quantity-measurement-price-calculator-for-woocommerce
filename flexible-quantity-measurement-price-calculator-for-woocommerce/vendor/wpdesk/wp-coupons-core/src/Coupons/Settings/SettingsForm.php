...
namespace WPDesk\Library\WPCoupons\Settings;

use WPDesk\Library\WPCoupons\CouponsIntegration;
...
use WPDesk\Library\WPCoupons\Settings\Tabs\SettingsTab;
...
class SettingsForm implements Hookable {
	...
	public function hooks() {
		add_action(
			...
			function () {
				add_submenu_page(
					self::MENU_PAGE_URL,
					esc_html__( 'Settings', 'wp-coupons-core' ),
					esc_html__( 'Settings', 'wp-coupons-core' ),
					'manage_options',
					self::SETTINGS_SLUG,
	...
	public function save_settings_action() {
		...
		$tab_data       = isset( $_POST[ $tab::get_tab_slug() ] ) ? wp_unslash( $_POST[ $tab::get_tab_slug() ] ) : ''; //phpcs:ignore
		...
		if ( ! empty( $tab_data ) && $nonce ) {
			...
			new Notice( esc_html__( 'Your settings have been saved.', 'wp-coupons-core' ), Notice::NOTICE_TYPE_SUCCESS );
		} else {
	...
	private function save_tab_data( array $post_data ) {
		foreach ( $post_data as $key => $value ) {
			if ( $key === '_empty_value' || $key === '' ) {
				continue; // Prevent save values for pro field.
			}
			if ( is_array( $value ) ) {
				$value = array_filter(
					$value,
	...
	private function get_active_tab(): SettingsTab {
		$selected_tab = isset( $_GET['tab'] ) ? sanitize_key( $_GET['tab'] ) : null; //phpcs:ignore
	...
	private function get_renderer() {
		 ...
		 * @since 3.0.0
		 */
		$resolver_list = (array) apply_filters(
			'fcpdf/settings/template_resolvers',
			[
				new DirResolver( __DIR__ . '/Views' ),
