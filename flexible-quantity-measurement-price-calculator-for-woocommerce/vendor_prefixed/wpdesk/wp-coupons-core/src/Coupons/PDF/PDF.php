...
use WC_Coupon;
use WDFQVendorFree\WPDesk\Library\CouponInterfaces\EditorIntegration;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Integration\Helper;
...
use WDFQVendorFree\WPDesk\Library\CouponInterfaces\Shortcode;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Product\ProductEditPage;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Shortcodes\Shortcodes;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Shortcodes\ShortcodeDataContainer;
use WDFQVendorFree\WPDesk\View\Renderer\Renderer;
use WC_Order;
use WC_Order_Item;
use WC_Order_Item_Product;
use WC_Product;
use WDFQVendorFree\WPDesk\Library\WPCoupons\Integration\ShortCodeReplacer;
/**
 * Render PDF
 *
...
class PDF
{
    ...
    private function prepare_template_data(int $template_id, WC_Order $order, WC_Order_Item $item, array $product_fields_values): array
    {
        ...
        if ($item instanceof WC_Order_Item_Product) {
            ...
            if (empty($template_meta) || !is_array($template_meta)) {
                throw new RuntimeException(esc_html__('Unknown template', 'flexible-quantity-measurement-price-calculator-for-woocommerce'));
            }
            $product = $item->get_product();
            $coupon = new WC_Coupon($coupon_id);
            if (!$coupon->get_id()) {
                throw new RuntimeException(esc_html__('Coupon doesn\'t exists.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'));
            }
    ...
    public function string_output($order_id, $item_id): string
    {
        ...
        if (defined('WDFQVendorFree\FLEXIBLE_COUPONS_DEBUG')) {
