...
use WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\CustomUnitsPage;
class Units
{
    ...
    {
        $result = [];
        $custom_units = \get_option(\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\CustomUnitsPage::OPTION_NAME, []);
        if (empty($custom_units)) {
            return $result;
        }
        foreach ($custom_units as $unit) {
            $unit_name = $unit[\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\Hookable\Settings\CustomUnitsPage::FORM_UNIT_NAME];
            $result[$unit_name] = ['label' => $unit_name, 'options' => [$unit_name]];
        }
        return $result;
    }
    /**
