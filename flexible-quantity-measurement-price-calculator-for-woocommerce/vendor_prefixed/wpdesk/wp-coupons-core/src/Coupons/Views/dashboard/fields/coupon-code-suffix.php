...
\woocommerce_wp_text_input(['id' => 'fc_coupon_code_suffix' . $loop_id, 'name' => 'fc_coupon_code_suffix' . $loop_name, 'value' => $value, 'label' => \esc_html__('Coupon code suffix', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'desc_tip' => \true, 'description' => \esc_html__('Define the suffix which will be used as a end of your coupon code. Leave empty if you don’t want to use the suffix. Use {order_id} shortcode if you want to use the order number.', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), 'wrapper_class' => !$is_premium ? 'read-only coupon-code-settings' : 'coupon-code-settings', 'custom_attributes' => $custom_attributes]);
