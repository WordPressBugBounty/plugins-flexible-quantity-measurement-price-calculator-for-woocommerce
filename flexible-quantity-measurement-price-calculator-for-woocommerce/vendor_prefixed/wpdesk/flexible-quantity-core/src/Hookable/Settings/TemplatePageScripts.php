...
class TemplatePageScripts implements Hookable
{
    /**
     * @var string
     */
    ...
    {
        global $post;
        if (!$post instanceof \WP_Post || $post->post_type !== FQTemplateType::POST_TYPE) {
            return;
        }
        ...
        \wp_localize_script('fq-admin', 'fq_admin_params', [
            ...
            'template_id' => $post->ID,
            'action_get_options' => SelectionAjax::ACTION_GET_OPTIONS,
            'action_get_selected' => SelectionAjax::ACTION_GET_SELECTED,
            'selection_nonce' => \wp_create_nonce(SelectionAjax::NONCE_CONTEXT),
            // old
            'dimensions_nonce' => \wp_create_nonce(DimensionsAjax::NONCE_CONTEXT),
            'action_get_dimensions' => DimensionsAjax::ACTION,
            ...
            'is_variable_product_with_stock_managed' => Helper::is_variable_product_with_stock_managed(wc_get_product($post)),
        ]);
    }
}
